...
import {
  Heart, MapPin, Edit3, Camera, Flame, Zap, TrendingUp, Users,
  ArrowRight, CheckCircle, AlertCircle, X, Sparkles, Award,
  Compass, Loader, Plus, ChevronLeft, ChevronRight, Maximize2,
...
const Badge = ({ icon: Icon, label, colorClass = "bg-gray-800 text-gray-300" }) => (
  <span className={`px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1.5 ${colorClass} border border-white/5`}>
...
const FullscreenImageViewer = ({ images, initialIndex, isOpen, onClose }) => {
  ...
  useEffect(() => { setCurrentIndex(initialIndex || 0); }, [initialIndex, isOpen]);
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
    >
      <button onClick={onClose} className="absolute top-6 right-6 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition">
        <X className="w-6 h-6" />
      </button>
      ...
      {images.length > 1 && (
        <>
          <button onClick={(e) => { e.stopPropagation(); prevImage(); }} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white">
            <ChevronLeft className="w-8 h-8" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); nextImage(); }} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white">
...
// PROFILE HEADER
// ============================================================================
const ProfileHeader = ({ profile, onEditClick, onAddPhotos, onCoverSelect, openViewer }) => {
  ...
  return (
    <Card className="relative overflow-visible mb-8">
      ...
      <div className="h-48 sm:h-64 w-full relative group">
        ...
        <input ref={coverInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && onCoverSelect?.(e.target.files[0])} />
      </div>
      ...
      <div className="px-6 pb-6 sm:px-10 sm:pb-8 relative z-20 -mt-20 flex flex-col md:flex-row items-start md:items-end gap-6">
        ...
        <div className="relative group">
          ...
          >
            {profile.profilePic ? (
              <img src={profile.profilePic} alt={profile.name} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
            ) : (
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight flex items-baseline gap-3">
                {profile.name}
                <span className="text-xl sm:text-2xl font-normal text-gray-500">{profile.age}</span>
              </h1>
            ...
            <div className="flex gap-3 mt-2 md:mt-0">
              ...
              <input ref={fileInputRef} type="file" multiple accept="image/*" className="hidden" onChange={(e) => onAddPhotos?.(e.target.files)} />
            </div>
          </div>

          {/* Tags */}
          <div className="flex flex-wrap gap-2 mt-4">
             {profile.gender && <Badge icon={User} label={profile.gender} colorClass="bg-blue-500/10 text-blue-400 border-blue-500/20" />}
             {profile.preference && <Badge icon={Heart} label={profile.preference} colorClass="bg-pink-500/10 text-pink-400 border-pink-500/20" />}
             {profile.course && <Badge icon={GraduationCap} label={profile.course} />}
             {profile.branch && <Badge icon={Briefcase} label={profile.branch} />}
             {profile.year && <Badge icon={BookOpen} label={`Year ${profile.year}`} />}
          </div>
        </div>
      </div>
      ...
      {profile.bio && (
        <div className="px-6 pb-6 sm:px-10 sm:pb-8 border-t border-gray-800 pt-6">
          <p className="text-gray-300 leading-relaxed max-w-4xl">{profile.bio}</p>
        </div>
      )}
    </Card>
  );
...
const StatsSection = ({ stats }) => {
  const items = [
    { label: "Perfect Matches", value: stats?.perfectMatches || 0, icon: Flame, color: "text-rose-500", bg: "bg-rose-500/10" },
    { label: "Great Matches", value: stats?.greatMatches || 0, icon: Zap, color: "text-amber-500", bg: "bg-amber-500/10" },
    { label: "Total Matches", value: stats?.totalMatches || 0, icon: Users, color: "text-violet-500", bg: "bg-violet-500/10" },
    { label: "Compatibility", value: `${stats?.avgCompatibility || 0}%`, icon: TrendingUp, color: "text-emerald-500", bg: "bg-emerald-500/10" },
  ];

  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      {items.map((item, idx) => (
        <Card key={idx} className="p-5 flex items-center gap-4 hover:border-gray-700 transition-colors">
    ...
    <Card className="p-6 h-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {images.map((img, idx) => (
          ...
          >
            <img src={img} alt="" className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition" />
        ...
        {[...Array(Math.max(0, 6 - images.length))].map((_, i) => (
           <div key={`empty-${i}`} className="aspect-square rounded-lg bg-gray-800/30 border border-gray-800/50" />
        ))}
      </div>
    </Card>
  );
...
const InsightsSection = ({ report, navigate }) => {
  if (!report) {
    return (
      <Card className="p-8 text-center border-dashed border-gray-700 h-full flex flex-col items-center justify-center">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">Unlock Insights</h3>
        <p className="text-gray-400 text-sm mb-6">Take the personality quiz to see analysis.</p>
        <button onClick={() => navigate("/personality-report")} className="px-6 py-2 bg-violet-600 text-white rounded-lg text-sm font-medium">
          Start Quiz
        </button>
      </Card>
    );
  ...
  const personalityType = aiGeneratedReport?.personalityType?.name || report.personalityProfile || "Unknown";
    ...
    <div className="space-y-6 h-full">
      ...
      <Card className="p-6 bg-gradient-to-br from-gray-900 to-gray-900 border-violet-500/20">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
           <div className="bg-gray-800/50 p-3 rounded-lg border border-gray-700">
              ...
              <div className="text-sm text-gray-200 font-medium truncate">{aiGeneratedReport?.relationshipApproach || "N/A"}</div>
           </div>
           <div className="bg-gray-800/50 p-3 rounded-lg border border-gray-700">
              <div className="text-xs text-gray-500 mb-1">Conflict Style</div>
              <div className="text-sm text-gray-200 font-medium truncate">{aiGeneratedReport?.conflictResolutionStyle || "Balanced"}</div>
           </div>
        </div>
      </Card>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-3">
          ...
          {(aiGeneratedReport?.developmentAreas || []).slice(0, 2).map((s, i) => (
             <div key={i} className="flex items-center gap-3 text-sm text-gray-300">
        ...
        <button onClick={() => navigate("/personality-report")} className="w-full mt-6 py-2 text-xs font-semibold text-gray-400 hover:text-white border border-gray-700 rounded-lg hover:bg-gray-800 transition">
          View Full Report
        </button>
      </Card>
    </div>
...
const EditProfileModal = ({ profile, isOpen, onClose, onSave, isLoading }) => {
  ...
  const inputClass = "w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none transition";
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={formData.age || ""} onChange={(e) => setFormData({...formData, age: e.target.value})} className={inputClass} />
            </div>
          </div>
          ...
          <div>
             ...
             <textarea rows={4} value={formData.bio || ""} onChange={(e) => setFormData({...formData, bio: e.target.value})} className={inputClass} />
          </div>
        ...
        <div className="p-6 border-t border-gray-800 bg-gray-900/50">
          ...
          >
            {isLoading ? <Loader className="animate-spin w-5 h-5" /> : "Save Changes"}
          </button>
        </div>
      </motion.div>
    </div>
...
function ModernProfilePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [profileRes, reportRes, matchesRes] = await Promise.all([
        ...
        fetch(`${BASE_URL}/api/personality/my-report`, { credentials: "include" }),
        fetch(`${BASE_URL}/api/compatibility/all-matches`, { credentials: "include" }).catch(() => null)
      ]);
      ...
      if (matchesRes && matchesRes.ok) {
        ...
        const avg = matches.length ? Math.round(matches.reduce((s, m) => s + m.compatibility, 0) / matches.length) : 0;
        setStats({
          perfectMatches: matches.filter(m => m.compatibility >= 80).length,
          greatMatches: matches.filter(m => m.compatibility >= 60 && m.compatibility < 80).length,
  ...
  const handleUpdate = async (updatedFields, newFiles) => {
    ...
    try {
      ...
      if(newFiles) Array.from(newFiles).forEach(f => fd.append('morePics', f));
  ...
  const handlePhotoUpload = async (files) => {
      ...
      await fetch(`${BASE_URL}/api/profile/${profile._id}/photos`, { method: "POST", body: fd, credentials: "include" });
      fetchData();
  }
  ...
  const handleCoverUpload = async (file) => {
      ...
      await fetch(`${BASE_URL}/api/profile/${profile._id}/cover`, { method: "PUT", body: fd, credentials: "include" });
      fetchData();
  }

  const allImages = profile ? [profile.profilePic, ...(profile.morePics || [])].filter(Boolean) : [];

  if (isLoading) return <div className="min-h-screen bg-gray-950 pt-24"><LoadingSkeleton /></div>;
  if (!profile) return <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center">No Profile Found</div>;
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 pb-20 pt-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <ProfileHeader 
          ...
          openViewer={(idx) => { setViewerIndex(idx); setViewerOpen(true); }}
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 h-full">
             <PhotoGrid images={allImages} onPhotoClick={(idx) => { setViewerIndex(idx); setViewerOpen(true); }} />
          </div>
        </div>
      </div>
      ...
      {/* Modals */}
      <AnimatePresence>
        <FullscreenImageViewer images={allImages} initialIndex={viewerIndex} isOpen={viewerOpen} onClose={() => setViewerOpen(false)} />
      </AnimatePresence>
      
      {isEditing && (
        <EditProfileModal profile={profile} isOpen={isEditing} onClose={() => setIsEditing(false)} onSave={handleUpdate} isLoading={isSaving} />
      )}
    </div>
  );
}
